    ...
    if (isset($_SESSION['id_usuario'])) {
        ...
        switch ($_GET['action']) {
            case 'readUsers':
                ...
                break;
            case 'readUsersClients':
                if ($usuario->readAllClients()) {
                    $result['status'] = 1;
                    $result['message'] = 'Debe autenticarse para ingresar';
                } else {
                    $result['exception'] = 'Debe crear un usuario para comenzar';
            ...
            case 'signup':
                ...
                if (!$usuario->setUser($_POST['usuario'])) {
                    $result['exception'] = 'Alias incorrecto';
                } elseif ($_POST['contrasenia'] != $_POST['conContra']) {
                    $result['exception'] = 'Claves diferentes';
                } elseif (!$usuario->setClave($_POST['contrasenia'])) {
                    $result['exception'] = Validator::getPasswordError();
                } elseif (!$usuario->setEstado($_POST['estado'])) {
                    $result['exception'] = 'No hay estado';
                } elseif ($usuario->createRow()) {
                    $result['status'] = 1;
            ...
            case 'login':
                ...
                if (!$usuario->checkUser($_POST['usuario'])) {
                    $result['exception'] = 'Alias incorrecto';
                } elseif ($usuario->checkPassword($_POST['contrasenia'])) {
                    $result['status'] = 1;
